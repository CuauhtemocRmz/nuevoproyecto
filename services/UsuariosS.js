...
class UsuariosService {
  ...
  async obtenerUsuario(id) {
    let resultado;
    let sentencia = 'select us.id as udId,us.usuario,us.pass' +
      ',tp.id as tpId,tp.identificador,tp.identificador_numerico,tp.descripcion' +
      ',dg.id as dgId,dg.nombre,dg.direccion,dg.telefono,dg.email' +
      ',ui.id as userInfoId ' +
      'from usuarios_info ui ' +
      'inner join usuarios us on (us.id=ui.usuarios_id) ' +
      'inner join tipo_persona tp on (tp.id=ui.tipo_persona_id) ' +
  ...
  async actualizarUsuario(id, body) {
    ...
    sentencia = 'update datos_generales set nombre=:nombre,direccion=:direccion,telefono=:telefono,email=:email '  +
    'where id=:id and status != 0';
